...
export function PortfolioDashboard({ inputs, model, assetPortfolio, onOpenAssetInput, onReset }: PortfolioDashboardProps) {
  ...
  return (
    <main className="no-scrollbar flex-1 overflow-y-auto bg-slate-50 px-5 py-6 pb-24">
      <Card className="mb-6">
        ...
        <div className="mb-6 flex justify-center">
          <div className="h-40 w-40">
            <PortfolioChart 
              ...
              expectedReturnPercent={hasPortfolioAssets && !showRecommendedChart ? undefined : model.expectedReturnPercent}
            />
          </div>
        </div>
      ...
      {showMptInfo ? (
        <div className="fixed inset-0 z-40 bg-slate-900/20 flex items-end backdrop-blur-[1px]" onClick={() => setShowMptInfo(false)}>
          <div
            className="no-scrollbar w-full max-h-[75vh] overflow-y-auto rounded-t-2xl border border-b-0 border-slate-200 bg-white p-6 shadow-2xl"
          ...
          >
            ...
            <div className="rounded-xl border border-blue-100 bg-blue-50/60 p-4">
              <h4 className="mb-2 flex items-center gap-1 text-xs font-extrabold text-blue-900">
                <GraduationCap size={14} />
                AI 포트폴리오 추천 엔진 브리핑
              </h4>
              <p className="mb-3 text-[11px] leading-relaxed text-slate-700">{model.xaiSummary}</p>
              <ul className="space-y-1 rounded-lg border border-blue-200 bg-white p-3 text-[10px] text-slate-600">
