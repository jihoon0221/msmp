...
import { relatedNewsArticles } from "../../constants/exploreArticles";
...
export function ExploreView({ riskProfile }: ExploreViewProps) {
  ...
  const articles = relatedNewsArticles;  
    ...
    <main className="no-scrollbar flex-1 overflow-y-auto bg-slate-50 px-5 py-5 pb-24">
      ...
      <div className="mb-4">
        <p className="mb-1 text-[10px] text-slate-500">
           현재 나의 보유 종목 코드와 종목명 기반으로 선별한 관련 뉴스입니다.
        </p>
...
function ArticleHero({ article }: { article: RelatedNewsArticle }) {
  return (
    <a
    ...
    >
      <div className="min-w-0 flex-1 pr-2">
        ...
        <p className="line-clamp-1 mt-1 text-[9px] text-slate-400">{article.summary}</p>
      </div>
